...
def conservative_downscale_array(s5p_path: Path, grid: GeoGrid, weights: np.ndarray, band: int = 1) -> Tuple[np.ndarray, object, np.ndarray]:
    ...
    src = rasterio.open(s5p_path)
    input_band_array = src.read(band).astype(float)
    nodata = src.nodata
    src_crs = src.crs
    if src_crs is None:
        src.close()
    ...
    return out, src, input_band_array
...
def conservative_downscale(
    ...
    out, src, input_band_array = conservative_downscale_array(s5p_path, grid, weights, band=band)
    try:
        write_pollutant_raster(output_path, grid, out, pollutant=pollutant, source_band=band)
        if validate:
            return validate_conservation(src, input_band_array, out, grid, weights)
        return None
    finally:
        src.close()

def validate_conservation(src, input_band_array: np.ndarray, fine: np.ndarray, grid: GeoGrid, weights: np.ndarray) -> dict:
...
def main() -> None:
    ...
    weights = build_weights(grid, met)
    report = {
        "grid": grid.as_dict(),
    ...
    if args.groundtruth_csv:
        ...
        base_field, base_src, base_input_band_array = conservative_downscale_array(args.input_tif, grid, weights, band=args.input_band)
        try:
            ...
            final_field, final_src, final_input_band_array = conservative_downscale_array(args.input_tif, grid, weights, band=args.input_band)
            final_field = apply_deblock(final_field, weights)
            try:
                pred_after, _, _ = sample_grid_values(final_field, grid, stations.x, stations.y)
                after = station_metrics(stations.value, pred_after)
                ...
                stats = validate_conservation(final_src, final_input_band_array, final_field, grid, weights) if args.validate else None
            finally:
                final_src.close()
            ...
            report.update({
                ...
                "background_value": background,
                "station_metrics_before": before,
                "station_metrics_after": after,
                "station_correction": corr_info,
            })
            print("Station metrics before correction:", json.dumps(before, indent=2))
            print("Station metrics after correction:", json.dumps(after, indent=2))
        finally:
            base_src.close()
    else:
        ...
        out, src, input_band_array = conservative_downscale_array(args.input_tif, grid, weights, band=args.input_band)
        try:
            out = apply_deblock(out, weights)
            write_pollutant_raster(
                args.output_tif,
                grid,
                out,
            ...
            stats = validate_conservation(src, input_band_array, out, grid, weights) if args.validate else None
        finally:
            src.close()
